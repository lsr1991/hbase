...
## 第二部分 阅读摘要
  ...
  - **HTable、HTablePool类及其实例**[1.p68;1.p191]
   ...
   - HTablePool类及其实例：为什么使用HTablePool类？因为HTable类不是线程安全（什么是线程安全？）的，所以在多线程环境中复用HTable实例会出现问题，于是得为每一个线程创建一个HTable实例。
  ...
  - **region拆分的过程**[1.p310~311]
   - 父region目录创建splits目录
   - 关闭父region，不接受请求
   - splits目录中创建子region的目录
   - 将子region目录移到表目录下
   - .META.表中父region状态更新，记录父region和子region的信息
   - 在子region目录下创建父region存储数据的引用文件（文件名中散列值与父region相同）
   - regionserver打开两个子region，更新.META.表中子region的信息，使其上线
   - 将父region存储文件异步重写到子region中.tmp目录下，边合并边重写
   - 生成的重写后的文件自动取代引用文件
